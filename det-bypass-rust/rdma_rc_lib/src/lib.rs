...
fn send_packets<T, F>(iters: u64, interval: u64, mut context: T, fun: F)
...
fn start_sending_packets<T, F>(iters: u64, interval: u64, ctx: T, fun: F) -> std::thread::JoinHandle<()> 
    where T: post_context::PostContext+'static,
          F: Fn(u64, &mut T) + Send+'static
{
