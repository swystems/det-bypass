...
pub struct PPContextBuilder<'a>{
    ...
    recv_buf: Option<[u8;consts::PACKET_SIZE]>,
    recv_size: Option<usize>,
    send_buf: Option<[u8;consts::PACKET_SIZE]>,
...
impl <'a>PPContextBuilder<'a>{
    ...
    pub fn recv_buf(mut self, recv_buf: [u8;consts::PACKET_SIZE], size: usize) -> PPContextBuilder<'a>{
        self.recv_buf = Some(recv_buf);
        self.recv_size = Some(size);
        self
    }

    pub fn send_buf(mut self, send_buf: [u8; consts::PACKET_SIZE], size: usize) -> PPContextBuilder<'a>{
    ...
    pub fn build(self) -> Result<PingPongContext, std::io::Error>{
        ...
        let recv_mr = unsafe {
            rdma::mr::MemoryRegion::register(&pd, self.recv_buf.unwrap().as_mut_ptr(), consts::PACKET_SIZE ,AccessFlags::LOCAL_WRITE, ())?
        }; 
        let send_mr = unsafe {
            rdma::mr::MemoryRegion::register(&pd, self.send_buf.unwrap().as_mut_ptr(), consts::PACKET_SIZE, AccessFlags::LOCAL_WRITE, ())?
        };
