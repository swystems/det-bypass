...
fn main() {
    let matches = Command::new("RC PingPong client").version("1.0").about("")
        ...
        .arg(arg!(-g --gidx <IDX> "Group index to attach the XDP program to").value_parser(value_parser!(u64)))
    ...
    let gidx: u32 = *matches.get_one::<u32>("gidx").unwrap();
    ...
    rdma_rc_lib::run_client(ib_devname, gidx, interval, iters, server_ip, persistence_flag);    
}
