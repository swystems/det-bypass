...
impl Config {
    pub fn new(ifname: String, ifindex: u32, iters: u64) -> Self{
        ...
        Config{xdp_flags, ifindex, ifname, iters, xsk_bind_flags, xsk_if_queue: 0, xsk_poll_mode: false}
    }
}
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let mut bpf = Bpf::load(include_bytes_aligned!(
        "../../target/bpfel-unknown-none/debug/xsk-pingpong-server"
    ))?;
    #[cfg(not(debug_assertions))]
    let mut bpf = Bpf::load(include_bytes_aligned!(
        "../../target/bpfel-unknown-none/release/xsk-pingpong-server"
    ))?;
    ...
    let cfg = Config::new(opt.iface.clone(), index, opt.packets);
    ...
    rx_and_process(cfg.xsk_poll_mode, s, opt.packets )?;
...
pub fn rx_and_process(poll_mode: bool, xsk_socket:std::sync::Arc<std::sync::Mutex<XskSocketInfo>>, iters: u64) -> Result<(), std::io::Error>{
    ...
    loop{
        ...
        let interrupt = handle_receive_packets(&mut socket, iters)?;
        if interrupt{
            break;
        }
    }
...
pub fn handle_receive_packets(xsk: &mut XskSocketInfo, iters: u64) -> Result<bool, std::io::Error>{
    let mut rec = 0;
    ...
    for i in 0..rec{
        unsafe{
            ...
            let (res, interrupt) = process_packet(xsk, (*ring).addr, (*ring).len, iters);
            inter = interrupt;
            if !res{
...
pub fn process_packet(xsk: &mut XskSocketInfo, addr: u64, len: u32, iters: u64) -> (bool, bool){
    let receive_timestamp = common::utils::get_time_ns();
    let pkt=  unsafe{libxdp::xsk_umem__get_data( xsk.umem.buffer.as_mut_ptr() as *mut libc::c_void, addr)};
    let eth_len = network_types::eth::EthHdr::LEN;
    let ip_len = network_types::ip::Ipv4Hdr::LEN;
    let pp_len =  PingPongPayload::LEN;
    
    if len < (eth_len + ip_len + pp_len) as u32{
        eprintln!("Received packet is too small");
        return (false, false);
    }
    let mut interrupt = false;
    let eth = pkt as *mut network_types::eth::EthHdr;
    let ip = unsafe{pkt.offset(eth_len as isize)} as *mut network_types::ip::Ipv4Hdr;
    ...
    unsafe{
        ...
        let tmp_mac = (*eth).dst_addr;
        (*eth).dst_addr = (*eth).src_addr;
        (*eth).src_addr = tmp_mac;

        let tmp_ip = (*ip).dst_addr;
        (*ip).dst_addr = (*ip).src_addr;
        (*ip).src_addr = tmp_ip;
        

        (*payload).ts[2] = common::utils::get_time_ns ();

        let mut packet = [0u8;common::consts::PACKET_SIZE];
        std::ptr::copy_nonoverlapping(eth as *const u8, packet.as_mut_ptr().add(0), eth_len);
        std::ptr::copy_nonoverlapping(ip as *const u8, packet.as_mut_ptr().add(eth_len), ip_len);
        std::ptr::copy_nonoverlapping(payload as *const u8, packet.as_mut_ptr().add(eth_len+ip_len), PingPongPayload::LEN);
        let _ = xsk_send_packet(xsk, packet, false, false);
...
pub fn xsk_send_packet(socket: &mut XskSocketInfo, packet: [u8;common::consts::PACKET_SIZE], is_umem_frame: bool, complete: bool)-> Result<(), std::io::Error>{
    ...
    unsafe {
        let dst = libxdp::xsk_umem__get_data( socket.umem.buffer.as_mut_ptr() as *mut libc::c_void, frame_desc.addr() as u64);
