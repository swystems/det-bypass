...
void extract_send_timestamps(msghdr *msg)
{
    ...
    for (cmsg = CMSG_FIRSTHDR(msg); cmsg; cmsg = CMSG_NXTHDR(msg, cmsg)) {
        if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SO_TIMESTAMPING) {
            ...
            if (err->ee_origin != SO_EE_ORIGIN_TIMESTAMPING) continue;
            if (idx != -1) {
...
void extract_send_timestamps(int sock, sockaddr_in *addr) {
    char control[2048];
    char buffer[0];
    struct iovec iov{.iov_base = buffer, .iov_len = 0,};
    msghdr msg
        {.msg_name = addr, .msg_namelen = sizeof(sockaddr_in), .msg_iov = &iov, .msg_iovlen = 1, .msg_control = control, .msg_controllen = sizeof(control),};
...
void receive(int socket, sockaddr_in *addr)
{
    ...
    char control[2048];
    struct iovec iov{.iov_base = buffer, .iov_len = PACKET_SIZE,};
    msghdr msg
        {.msg_name = addr, .msg_namelen = sizeof(sockaddr_in), .msg_iov = &iov, .msg_iovlen = 1, .msg_control = control, .msg_controllen = sizeof(control),};
...
void send(int socket, sockaddr_in *addr)
{
    ...
    struct iovec iov{.iov_base = buffer, .iov_len = PACKET_SIZE,};
    msghdr msg
        {.msg_name = addr, .msg_namelen = sizeof(sockaddr_in), .msg_iov = &iov, .msg_iovlen = 1, .msg_control = nullptr, .msg_controllen = 0};
...
int start_program(string ifname, string remote, u64 packets, u64 interval)
{
    ...
    sockaddr_in local_sockaddr = {.sin_family = AF_INET, .sin_port = htons(PORT), .sin_addr = {.s_addr = INADDR_ANY},};
